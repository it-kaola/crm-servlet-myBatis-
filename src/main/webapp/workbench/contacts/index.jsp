	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#searchBtn").click(function () {
				...
				// 展示列表
				showContactsList(1, $("#contactPage").bs_pagination('getOption', 'rowsPerPage'));
			});
		...
		function showContactsList(pageNo, pageSize){
			...
			$.ajax({
				url : "workbench/contacts/queryContacts.do",
				data : {
					"pageNo" : pageNo,
					"pageSize" : pageSize,
				...
				success : function (respData) {
					// respData 应该为 {"total" : xxx, dataList : [{联系人1}, {联系人2}, ....]}
					...
					$.each(respData.dataList, function (index, element) {
						...
						html += '<td><a style="text-decoration: none; cursor: pointer;" onclick="window.location.href=\'workbench/contacts/detail.jsp?'+ element.id +'\';">'+ element.fullname +'</a></td>';
						html += '<td>'+ element.customerId +'</td>'; // 这里需要用到表的连接查询
					...
					$("#contactPage").bs_pagination({
						...
						onChangePage : function(event, data){ // 这两个参数是分页插件提供的，不要擅自修改
							showContactsList(data.currentPage , data.rowsPerPage);
						}
					});
				}
			})
				  ...
				  <div class="form-group">
				    <div class="input-group">
				      ...
				      <select class="form-control" id="edit-source">
						  <option></option>
						  <:c:forEach item="${sourceList}" var="dicVal">
