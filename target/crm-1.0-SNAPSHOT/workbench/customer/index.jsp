	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#searchBtn").click(function () {
				...
				// 展现客户列表
				showCustomerList(1, $("#customerPage").bs_pagination('getOption', 'rowsPerPage'));
			});
		...
		function showCustomerList(pageNo, pageSize) {
			...
			$("#search-owner").val($("#hidden-phone").val());
			$("#search-website").val($("#hidden-website").val());

			$.ajax({
				url : "workbench/customer/queryCustomers.do",
				data : {
					"pageNo" : pageNo,
					"pageSize" : pageSize,
				...
				success : function (respData) {
					...
					$.each(respData.dataList, function (index, element) {
						...
						html += '<td><a style="text-decoration: none; cursor: pointer;" onclick="window.location.href=\'workbench/customer/detail.jsp?'+ element.id +'\';">'+ element.name +'</a></td>';
						html += '<td>'+ element.owner +'</td>'; // 这里的owner需要进行表的连接查询
